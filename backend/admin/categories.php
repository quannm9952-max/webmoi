...
$row  = $edit ? $m->findCategory($edit) : null;
    ...
    <div class="col-md-4">
        <div class="bg-white p-4 rounded-4 shadow-sm">
            <h5 class="fw-700 mb-3">
                <?= $row ? 'Sửa danh mục' : 'Thêm danh mục' ?>
            </h5>
            <form method="post">
                <input type="hidden" name="id" value="<?= h($row['id_danh_muc'] ?? 0) ?>">
                <div class="mb-3">
                    <label class="form-label">Tên danh mục</label>
                    <input name="name" class="form-control"
                           value="<?= h($row['ten_danh_muc'] ?? '') ?>"
                ...
                <?php if ($row): ?>
                <a href="<?= BASE_URL ?>/admin/categories.php" class="btn btn-outline-secondary w-100 mt-2">Hủy</a>
                <?php endif; ?>
            </form>
        </div>
    </div>
        ...
        <div class="bg-white p-4 rounded-4 shadow-sm table-responsive">
            <table class="table align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="fw-600"><?= h($r['ten_danh_muc']) ?></td>
                        <td>
                        ...
                        <td class="text-end">
                            <a href="<?= BASE_URL ?>/admin/categories.php?edit=<?= (int)$r['id_danh_muc'] ?>"
                    ...
                    <?php if (empty($rows)): ?>
                    <tr>
                        <td colspan="3" class="text-center text-muted py-5">Chưa có danh mục nào.</td>
                    </tr>
                    <?php endif; ?>
                </tbody>
            </table>
